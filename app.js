...
function reviveState(raw){ const base = freshState(); const settings={...base.settings,...(raw.settings||{})}; return {...base,...raw,settings,numbers:raw.numbers||base.numbers,summary:{...base.summary,...(raw.summary||{})},ladder:Array.isArray(raw.ladder)&&raw.ladder.length?raw.ladder:buildLadder(settings),activeTab:raw.activeTab||'sangram'}; }
...
function resolveNumber(side,num,notes){ const info=state.numbers[side][num]; if(info.status==='L'){ notes.push({title:'ZERO RULE', text:`${side}${num} treated as 0`}); return; }
...
function setupControls(){
  ...
  q('applyYantraBtn').addEventListener('click', ()=>{
    const s=state.settings; s.bankroll=Number(q('setBankroll').value)||30000; s.targetDollar=Number(q('setTargetDollar').value)||500; s.targetPercent=Number(q('setTargetPercent').value)||1.67; s.stopLoss=Number(q('setStopLoss').value)||2000; s.min=Number(q('setMin').value)||100; s.max=Number(q('setMax').value)||3000; s.coin=Number(q('setCoin').value)||100; s.targetNum=Number(q('setTargetNum').value)||500; s.doubleLadder=q('setDoubleLadder').value; s.keypadMode=q('setKeypadMode').value; s.maxSteps=Number(q('setMaxSteps').value)||30; s.reserve=Number(q('setReserve').value)||20000; s.capRule=q('setCapRule').value; state.ladder=buildLadder(s); state.liveBankroll=s.bankroll; renderAll(); showToast('YANTRA APPLIED','Settings saved');
  });
  ...
  q('resetLadderBtn').addEventListener('click', ()=>{ state.ladder=buildLadder(state.settings); renderAll(); showToast('SOPANA RESET','Default ladder restored'); });
