...
function renderSangram(){ q('bankValue').textContent=fmtMoney(state.liveBankroll); q('chakraValue').textContent=`Round : ${state.currentChakra}`; q('nextY').textContent=formatNextAhuti('Y'); q('nextK').textContent=formatNextAhuti('K'); q('nextT').textContent=`T ${nextExposureTotal()}`; const rows=currentKumbh()?.rows||[]; const last=rows[rows.length-1]; const lastText=last ? `${last.y ?? '-'} | ${last.k ?? '-'}` : '-'; const el=q('lastResultValue'); if(el) el.textContent=lastText; }
