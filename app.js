...
function setupControls(){
  ...
  q('applyYantraBtn').addEventListener('click', ()=>{
    const s=state.settings; s.bankroll=Number(q('setBankroll').value)||30000; s.targetDollar=Number(q('setTargetDollar').value)||500; s.targetPercent=Number(q('setTargetPercent').value)||1.67; s.stopLoss=Number(q('setStopLoss').value)||50000; s.min=Number(q('setMin').value)||100; s.max=Number(q('setMax').value)||3000; s.coin=Number(q('setCoin').value)||100; s.targetNum=Number(q('setTargetNum').value)||500; s.doubleLadder=q('setDoubleLadder').value; s.keypadMode=q('setKeypadMode').value; s.maxSteps=Number(q('setMaxSteps').value)||30; s.reserve=Number(q('setReserve').value)||20000; s.capRule=q('setCapRule').value; s.stopLossPerNumber=parseSignedInt(q('setStopLossPerNumber')?.value, -100); state.ladder=buildLadder(s); renderAll(); showToast('YANTRA APPLIED','Settings saved');
  });
  q('saveLadderBtn').addEventListener('click', ()=>{ let cumulative=0; document.querySelectorAll('[data-ladder-index]').forEach(inp=>{ const i=Number(inp.dataset.ladderIndex); const bet=Math.max(state.settings.coin, Number(inp.value)||0); inp.value=bet; cumulative += bet; state.ladder[i]={ step:`S${i+1}`, bet, winReturn: bet*9, netProfit:(bet*9)-cumulative, ifLoseTotal:-cumulative }; }); const lines=['ladder,step,bet', ...state.ladder.map((row,idx)=>`L1,S${idx+1},${row.bet}`)]; downloadFile('sopana-ladder.csv', lines.join('\n'), 'text/csv'); renderAll(); showToast('SOPANA SAVED','Editable ladder updated'); });
